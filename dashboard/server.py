...
async def post_broadcast(request):
    ...
    users=list(db.all_active_users()); sem=asyncio.Semaphore(25)
    async def _s(u):
        async with sem:
            try:
                async with httpx.AsyncClient(timeout=10) as c:
                    r=await c.post(f'https://api.telegram.org/bot{token}/sendMessage',json={'chat_id':u['user_id'],'text':message,'parse_mode':'HTML'})
                await asyncio.sleep(0.05); return 1 if r.status_code==200 else 0
            except: return 0
    res=await asyncio.gather(*[_s(u) for u in users])
    return json_resp({'sent':sum(res),'failed':len(res)-sum(res)})
