...
from keyboards import (
    ...
    nd_country_keyboard, nd_goal_keyboard, nd_confirm_keyboard,
)
from states import UserFlow, AdminFlow, AdGateStates, BMToolStates, NewDarkStates
...
from gates.new_dark_gate import NewDarkGate
from gates.base_gate import BANNER_PHOTO_PATH

# قيم افتراضية للتجربة المحلية — للإنتاج استخدم TELEGRAM_BOT_TOKEN و ADMIN_PASS في البيئة فقط
# ⚠️ استخدم متغيرات البيئة فقط
TOKEN      = os.environ.get('TELEGRAM_BOT_TOKEN', '').strip()
ADMIN_PASS = os.environ.get('ADMIN_PASS', '').strip()
ADMIN_CMD  = (os.environ.get('ADMIN_CMD', '') or 'admin').strip()
...
# حالات نتحقق فيها من message_id لمنع نقر أزرار بروكسي من رسالة قديمة فقط
_STRICT_MSG_CALLBACK_STATES = frozenset({
    'AdGateStates:waiting_proxy',
    'BMToolStates:waiting_proxy',
})


async def purge_tracked_bot_messages(bot, chat_id: int, msg_ids) -> None:
    """يحذف رسائل البوت المسجّلة في flow_msg_ids (مسار الإعلانات)."""
    if not msg_ids:
        return
    for mid in msg_ids:
        try:
...
async def _check_session(call: CallbackQuery, state: FSMContext) -> bool:
    """
    يتحقق من أن callback جاء من الرسالة الحالية أو من رسالة بوت مسجلة في نفس الجلسة.
    """
    data = await state.get_data()
    active_mid = data.get('active_msg_id')
    flow_ids = set(data.get('flow_msg_ids', []))
    if not active_mid and not flow_ids:
        return True
    if active_mid and call.message.message_id == active_mid:
        return True
    if call.message.message_id in flow_ids:
        return True
    await call.answer('⚠️ هذه الرسالة قديمة. يتم إعادة البوت إلى البداية الآن.', show_alert=True)
    await state.clear()
...
async def send_home(msg_or_call, user_id: int):
    ...
    if isinstance(msg_or_call, CallbackQuery):
        # نحاول نرسل صورة البانر مع القائمة الرئيسية
        if BANNER_PHOTO_PATH.exists():
            try:
                await msg_or_call.message.delete()
                await msg_or_call.message.answer_photo(
...
async def start(message: Message, state: FSMContext):
    ...
    await purge_tracked_bot_messages(message.bot, message.chat.id, prev.get('flow_msg_ids'))
    await state.clear()
    db.add_user(
...
async def redeem_start(call: CallbackQuery, state: FSMContext):
    ...
    # نحذف أي رسائل سابقة للبانر ونرسل رسالة جديدة
    if BANNER_PHOTO_PATH.exists():
        try:
...
# ⚠️ مهم: هذا الـ handler لازم يكون في الأولوية القصوى لأي رسالة ما دام المستخدم في حالة waiting_redeem
@dp.message(UserFlow.waiting_redeem)
async def redeem_code(message: Message, state: FSMContext):
    code  = message.text.strip()
    # Debug: نتأكد إن الكود وصل
    print(f"🔍 Redeem code received: '{code}' from user {message.from_user.id}")
    hours = db.use_code(code, message.from_user.id)
    if not hours:
        print(f"❌ Redeem failed for code: '{code}'")
        await message.answer(
            "❌ <b>الكود غير صالح أو تم استخدامه من قبل.</b>\n\nتأكد من الكود وحاول مرة أخرى.",
            reply_markup=back_home()
        )
        return
    until = db.add_subscription_hours(message.from_user.id, hours)
    await state.clear()
    print(f"✅ Redeem success! User {message.from_user.id} got {hours}h")
    
    # نرسل رسالة النجاح مع البانر
    if BANNER_PHOTO_PATH.exists():
        try:
...
async def enter_gate(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        active_msg_id=data.get('banner_msg_id', call.message.message_id),
        fixed_msg_id=data.get('banner_msg_id', call.message.message_id),
        flow_msg_ids=data.get('flow_msg_ids', []),
    )
    await call.answer()
...
async def admin_gen_code(call: CallbackQuery, state: FSMContext):
    await state.update_data(extend_uid=None)
...
async def admin_code_duration(message: Message, state: FSMContext):
    try:
        ...
        return
    data = await state.get_data()
    extend_uid = data.get('extend_uid')
    if extend_uid is not None:
        until = db.add_subscription_hours(int(extend_uid), hours)
        await state.clear()
        await message.answer(
            f"✅ <b>تم تمديد اشتراك المستخدم</b>\n\n"
            f"🆔 <code>{extend_uid}</code>\n"
            f"➕ أُضيفت <b>{hours}</b> ساعة\n"
            f"⏳ صالح حتى (UTC):\n<code>{until.isoformat(timespec='seconds')}</code>",
            reply_markup=admin_panel()
        )
        return
...
async def admin_manage_user(message: Message, state: FSMContext):
    ...
    until = db.add_subscription_hours(user_id, hours)
    await state.clear()
    days = hours // 24
    await message.answer(
...
async def admin_broadcast(message: Message, state: FSMContext):
    users=list(db.all_active_users()); txt=message.text; sem=asyncio.Semaphore(25)
    async def _snd(u):
        async with sem:
            try: await bot.send_message(u['user_id'],txt,parse_mode='HTML'); await asyncio.sleep(0.05); return 1
            except: return 0
    res=await asyncio.gather(*[_snd(u) for u in users]); sent=sum(res); failed=len(res)-sent
    await state.clear()
    await message.answer(f"📢 <b>تم الإرسال</b>\n✅ نجح: <b>{sent}</b>\n❌ فشل: <b>{failed}</b>\n📊 الإجمالي: <b>{len(users)}</b>", reply_markup=admin_panel())


# ── الإعدادات ──

@dp.callback_query(F.data == 'admin:set_botname')
async def admin_set_botname_info(call: CallbackQuery):
    await call.message.edit_text(
        "✏️ <b>اسم البوت</b>\n\n"
        "يُعرض من متغير البيئة <code>BOT_NAME</code> عند تشغيل العملية.\n"
...
async def admin_set_support_info(call: CallbackQuery):
    await call.message.edit_text(
        "🔗 <b>رابط الدعم</b>\n\n"
        "يُضبط عبر <code>SUPPORT_URL</code> في البيئة ثم إعادة التشغيل.",
        reply_markup=back_admin()
    )
    await call.answer()
...
async def admin_settings(call: CallbackQuery):
    dashboard_port = os.environ.get('PORT', os.environ.get('DASHBOARD_PORT', '5000'))
    await call.message.edit_text(
...
# == New Dark Handlers ==
@dp.callback_query(F.data=='proxy:auto',NewDarkStates.waiting_proxy)
async def nd_pxa(call:CallbackQuery,state:FSMContext):
    if not await _check_session(call,state): return
    await new_dark_gate.handle_proxy_auto(call,state,proxy_manager.choose()); await call.answer()

@dp.callback_query(F.data=='proxy:skip',NewDarkStates.waiting_proxy)
async def nd_pxs(call:CallbackQuery,state:FSMContext):
    if not await _check_session(call,state): return
    await new_dark_gate.handle_proxy_skip(call,state); await call.answer()

@dp.callback_query(F.data=='proxy:custom',NewDarkStates.waiting_proxy)
async def nd_pxc(call:CallbackQuery,state:FSMContext):
    if not await _check_session(call,state): return
    await new_dark_gate.update_banner_caption(call.message.bot,call.message.chat.id,state,'✏️ <b>أدخل البروكسي يدوياً:</b>\n\n<code>IP:PORT</code>',reply_markup=back_home(),parse_mode='HTML'); await call.answer()

@dp.callback_query(F.data=='proxy:back',NewDarkStates.waiting_cookies)
async def nd_pxb(call:CallbackQuery,state:FSMContext):
    if not await _check_session(call,state): return
    await new_dark_gate.handle_proxy_back(call,state); await call.answer()

@dp.message(NewDarkStates.waiting_proxy)
async def nd_pi(m:Message,state:FSMContext): await new_dark_gate.handle_proxy_custom(m,state)

@dp.message(NewDarkStates.waiting_cookies)
async def nd_ck(m:Message,state:FSMContext): await new_dark_gate.handle_cookies(m,state)

@dp.message(NewDarkStates.waiting_ad_account_id)
async def nd_ai(m:Message,state:FSMContext): await new_dark_gate.handle_ad_account_id(m,state)

@dp.message(NewDarkStates.waiting_page_id)
async def nd_pi2(m:Message,state:FSMContext): await new_dark_gate.handle_page_id(m,state)

@dp.callback_query(F.data.startswith('nd:country:'),NewDarkStates.waiting_country)
async def nd_co(call:CallbackQuery,state:FSMContext):
    if not await _check_session(call,state): return
    await new_dark_gate.handle_country(call,state,call.data.split(':',2)[2]); await call.answer()

@dp.callback_query(F.data.startswith('nd:goal:'),NewDarkStates.waiting_goal)
async def nd_gl(call:CallbackQuery,state:FSMContext):
    if not await _check_session(call,state): return
...
@dp.message(NewDarkStates.waiting_caption)
async def nd_ca(m:Message,state:FSMContext): await new_dark_gate.handle_caption(m,state)

@dp.message(NewDarkStates.waiting_daily_budget)
async def nd_bu(m:Message,state:FSMContext): await new_dark_gate.handle_daily_budget(m,state)

@dp.message(NewDarkStates.waiting_days)
async def nd_dy(m:Message,state:FSMContext): await new_dark_gate.handle_days(m,state)
...
async def main():
    ...
    print(f"🔑 الأمر السري: /{ADMIN_CMD}  |  لوحة الأدمن: {'مفعّلة' if ADMIN_PASS else 'معطّلة (اضبط ADMIN_PASS)'}")
